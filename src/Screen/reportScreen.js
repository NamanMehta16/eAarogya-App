...
const reportScreen = () => {
  ...
  return (
    <View style={{...styles.backgroundContainer, backgroundColor: reports.length ? '#eee' : '#fff'}}>
      <View style={{backgroundColor: '#0f4c75', marginTop: 100, padding: 50, alignItems: 'center', width: '100%', borderBottomLeftRadius: 70}}>
      ...
      <View style={{width: '100%', height: '100%', marginTop: 20}}>
          <Text style={{margin: 5, fontSize: 30, color: '#0f4c75', fontFamily: 'Ubuntu_700Bold'}}>{type}</Text>
          {reports.length==0 ? <Image source={Report} style={{height: 350, width: 350, alignSelf: 'center', marginTop: 20}} /> :
          <FlatList 
          ...
          renderItem={({item}) => (
            <View style={styles.box}>
              <View style={{flexDirection: 'row', alignItems: 'center'}}>
                <FontAwesome name="calendar-o" size={15} color="#ccc" style={{marginRight: 10}} />
                <Text style={styles.boxText}>{item.date}</Text>
              </View>
              <Text style={{...styles.boxText, fontSize: 20}}>{item.info}</Text>
            </View>
          )}
